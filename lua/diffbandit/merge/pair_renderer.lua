-- Build a minimal render context for one merge-side diff pair.
-- Uses session.render_host methods only — not the full Session class.
...
function M.from_pair(owner, id_suffix, pair, left_source, right_source, buffers, windows, opts)
  ...
  setmetatable(renderer, { __index = render_host })
